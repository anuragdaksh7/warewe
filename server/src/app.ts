...
app.use(
  morgan((tokens, req, res) => {
    return [
      tokens["colored-method"] ? tokens["colored-method"](req, res) : "", // Colored HTTP method
      chalk.white(tokens.url?.(req, res)), // URL
      tokens["colored-status"] ? tokens["colored-status"](req, res): "", // Colored status
      chalk.gray((tokens.res ? tokens.res(req, res, "content-length"): "0") + "b"), // Size
      "-",
      chalk.magenta(tokens["response-time"] ? tokens["response-time"](req, res) : "" + " ms"), // Response time
      chalk.blue(tokens["remote-addr"] ? tokens["remote-addr"](req, res) : ""), // IP
      chalk.gray("[" + (tokens["iso-date"] ? tokens["iso-date"](req, res) : "") + "]"), // Date
    ].join(" ");
  }),
);
...
// app.use("/asd", async (req, res) => {
//   const r = await req.em.find(RequestHistory)
//   res.send(r)
// })
